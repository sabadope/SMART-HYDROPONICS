...
BEGIN
    ...
    -- Insert profile with ALL registration form data
    INSERT INTO public.user_profiles (id, email, full_name, password, confirm_password)
    VALUES (NEW.id, NEW.email, user_full_name, user_password, user_confirm_password);

    -- Log successful profile creation with all registration data (passwords are SHA-256 hashed)
    RAISE LOG 'User profile created - ID: %, Email: %, Full Name: %, Password: [SHA-256 HASHED], Confirm: [SHA-256 HASHED]',
